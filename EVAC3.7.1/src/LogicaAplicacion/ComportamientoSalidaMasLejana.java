...
public class ComportamientoSalidaMasLejana extends Comportamiento {
    
    private int puertaMasLejana(Point nodo, AutomataCelular ac) { //MODIFICADO PARA ENCONTAR LA SALIDA CON MENOR DISTANCIA
        int puerta = 0;                                            //AHORA YA NO EXISTE EL CAMPO DISTANCIA SALIDA QUE GUARDABA 
        Double distancia = 0.0;                                    //LA DISTANCIA SOLO HACIA LA SALIDA MAS CERCANA (05-10-2015)
    ...
    public boolean ejecutarComportamiento(AutomataCelular ac, AutomataCelular ac2, AutomataCelular acAmbiente, Point nodo) {
        ...
        if ( this.getVecinos().size()>0 && (ac.getCelda(nodo.y, nodo.x).getAgente().getContadorVelocidad() < ac.getCelda(nodo.y, nodo.x).getAgente().getVelocidad()) ) {
            ...
            int i;
            try {
                if((this.puertaMasLejana(nodo, ac)-1 ) ==-1){
                    ...
                    if(ac.getCelda(nodo.y, nodo.x).getAgente().getSalidaElegida()==-1){
                        ac.getCelda(nodo.y, nodo.x).getAgente().setSalidaElegida(this.puertaMasLejana(nodo, ac));
                        ((Contador)ThreadSimulacion.intenciones.get( this.puertaMasLejana(nodo, ac)-1 )).incrementarContador();
                    }
                    
                }
            }
            catch (Exception ex) {
                System.out.println(this.puertaMasLejana(nodo, ac));
            ...
            if (ac.getCelda(nodo.y, nodo.x).getAgente().getTiempoNoActualizacion() < 3){
                if (this.getVecinosSalida().size() > 0) {
                    ...
                    ac.getCelda(nodo.y, nodo.x).getAgente().setDestino((Point)this.getVecinosSalida().get(randomDesicion));
                     ...
                     //CRISTIAN 18/04/2018 CODIGO PARA MEJORAR LA NAVEGACION
